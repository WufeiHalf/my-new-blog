...
这篇文章记录一次完整的 OpenClaw 技能体系落地过程。我在服务器上把 OpenClaw 从“能聊天”推进到“可持续执行工作流”，过程中先遇到 **SearXNG JSON 403**，随后补齐 **ClawdHub skill 安装链路**、**auto-updater / self-improving-agent** 和 **Agent Browser** 的运行环境。

为了避免泄露敏感信息，文中所有 token、内网地址、具体密钥都用占位符代替。

## 目标：把 OpenClaw 从“能聊天”变成“能持续产出”的工作流

这次目标很明确：OpenClaw 不只回答问题，还要在服务器上形成稳定的三段闭环：
...
这个问题说明了一件事：代理侧“看起来可用”的能力，可能被服务端默认安全策略直接限制。把这些约束写进技能文档，比靠记忆可靠。
...
这个错误的根因是：CLI 某段代码依赖 `undici`，而 npx 的临时环境没有完整带上依赖，导致进程启动即失败。
...
安装过程中有一个典型运维细节：系统会提示“内核已升级，建议重启以加载新内核”。这不是阻断项，但说明当前变更已经触达系统运行时依赖。若追求长期稳定，后续应安排一次计划内重启。
...
这轮工作表面上是在安装 skills，实质收获是把几个隐含规则显性化：
